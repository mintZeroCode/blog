...
`}
  ...
  console.log(c(a, b)); //20

`}
          </CodeBox>
        </Highlight>
        함수를 매개변수처럼 사용할 수 있고 매개변수로 사용할 값은 return 문에서
        리턴할 값입니다. 이예제는 함수를 표현식으로 사용한 것과 함수를 변수처럼
        사용한다는 것을 보여줍니다. <br />​ 다음으로, 자바스크립트에서 배열을
        사용한다면 일반적으로 배열의 요소로 변수를 사용한다고 생각하실겁니다.
        하지만 변수대신 함수를 배열 요소로 사용할 수 있습니다. <br />​ ​
        다음코드는 배열의 각요소에 변수를 저장한 코드입니다.
        <Highlight language="javascript">
          <CodeBox>
            {` let array = [1, 2, 3, 4, 5];
...
`}
        ...
        <br />
        다음코드는 배열의 각요소에 함수를 호출해서 저장한 코드입니다.
        <br />
        <Highlight language="javascript">
          <CodeBox>
            {` let a = () => 1;
